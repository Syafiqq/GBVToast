  ...
  struct ToastInlineLayoutTests {
    ...
    @Test(
      "Inline message is centered at semantic leading when CTA is taller",
      arguments: [LayoutDirection.leftToRight, .rightToLeft]
    )
    func messageIsCenteredAtSemanticLeading(layoutDirection: LayoutDirection) {
      let bounds = CGRect(x: 17, y: 29, width: 240, height: 72)
      let placement = toastInlineMessagePlacement(
        in: bounds,
        layoutDirection: layoutDirection
      )
      let messageFrame = CGRect(
        x: placement.point.x - placement.anchor.x * 120,
        y: placement.point.y - placement.anchor.y * 24,
        width: 120,
        height: 24
      )

      #expect(messageFrame.midY == bounds.midY)
      if layoutDirection == .leftToRight {
        #expect(messageFrame.minX == bounds.minX)
        #expect(placement.anchor == .leading)
      } else {
        #expect(messageFrame.maxX == bounds.maxX)
        #expect(placement.anchor == .trailing)
      }
    }
  }
