...
def build_html(pairs: list[SnapshotPair]) -> str:
    groups: list[str] = []
...
header{display:flex;align-items:baseline;justify-content:space-between;gap:12px;margin-bottom:12px}.device{display:block;color:#98a2b8;font-size:12px;margin-top:4px}code{color:#98a2b8;font-size:12px}.two-up{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.two-up figure{margin:0;background:repeating-conic-gradient(#181d26 0 25%,#141922 0 50%) 0/20px 20px;border:1px solid #303746;border-radius:9px;padding:10px}.two-up figcaption{font-size:12px;font-weight:700;color:#cbd3e2;margin-bottom:8px}.two-up img{display:block;width:100%;height:auto}.empty{display:grid;place-items:center;align-content:center;gap:8px;min-height:140px;color:#7f899d;border:1px dashed #465064;border-radius:7px;background:#11151ccc;text-align:center;padding:16px}.empty small{max-width:30em}@media(max-width:800px){body{padding:14px}.two-up{grid-template-columns:1fr}}
...
def main() -> int:
    ...
    arguments.output.write_text(build_html(pairs), encoding="utf-8")
    print(f"Wrote {len(pairs)} comparisons to {arguments.output}")
    return 0
